class VendingMachine
  ...
  MONEY = [10, 50, 100, 500, 1000].freeze #0-1.10円玉、50円玉、100円玉、500円玉、1000円札を１つずつ投入できる。
  ...
  def slot_money(money)　　　　#0-2.投入は複数回できる。 1-1.想定外のもの（硬貨：１円玉、５円玉。お札：千円札以外のお札）が投入された場合は、投入金額に加算せず、それをそのまま釣り銭としてユーザに出力する。
...
# わかりやすいように出来るだけシンプルに書いてます.
# メソッドあるいはクラスだけをコメントアウトで設問を指摘しているのですが
# 他のメソッドとも連動して動くものもあるので、
# そこも注意して読んで欲しいです。
